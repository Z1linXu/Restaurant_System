...
grep -Fq 'production_control_checkout_sha' "$SCRIPT"
grep -Fq 'Production control checkout identity drifted' "$SCRIPT"
grep -Fq 'success::text' "$SCRIPT"
grep -Fq '.production-ops.lock' "$SCRIPT"
grep -Fq 'Production Flyway ledger is not exact V10' "$SCRIPT"
grep -Fq 'ops001-flyway-production-checksums.txt' "$SCRIPT"
grep -Fq 'FLYWAY_TARGET: "10"' "$ROOT/deployment/cloud/docker-compose.yml"
