...
grep -Fq 'backup_flyway_target' "$SCRIPT"
grep -Fq 'backup Flyway target is invalid' "$SCRIPT"
grep -Fq '/^[0-9]+[|]/' "$SCRIPT"
grep -Fq 'isolated restore Flyway ledger differs from exact backup target' "$SCRIPT"
grep -Fq 'flyway=%s-exact' "$SCRIPT"
