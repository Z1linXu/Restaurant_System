...
`owner-login-acceptance` is the bounded Phase A API action. It proves the approved
...
runtime business data. It does not replace browser acceptance.
`rotate-owner-credential` requires the old and a distinct, exact 20-character
new password only through the inherited private FD. It authenticates and
verifies the same exact synthetic Owner context, resets only that authenticated
Owner's credential through the existing staff-admin contract, then proves the
new credential through a second login/context/logout sequence. The caller must
retain the new value only in the approved server-private mode-0600 credential
artifact; the client never prints it. `prepare-target` proves Owner identity/workspace/overview, performs onboarding
