...
validate_inputs() {
  ...
  STAGING_ALLOWED_PRINTING_MODES="$(dotenv_value STAGING_ALLOWED_PRINTING_MODES || true)"
  STAGING_PRINTER_ENDPOINT_CONFIGURATION_ENABLED="$(dotenv_value STAGING_PRINTER_ENDPOINT_CONFIGURATION_ENABLED || true)"
  [[ -n "$STAGING_ALLOWED_PRINTING_MODES" ]] || STAGING_ALLOWED_PRINTING_MODES="DISABLED,MOCK"
  [[ -n "$STAGING_PRINTER_ENDPOINT_CONFIGURATION_ENABLED" ]] || STAGING_PRINTER_ENDPOINT_CONFIGURATION_ENABLED="false"
  case "$STAGING_PRINT_MODE/$STAGING_PRINTING_FEATURE_ENABLED" in
    DISABLED/false|MOCK/true) ;;
    *) die "Staging printing must use DISABLED/false or MOCK/true" ;;
  esac
  [[ "$STAGING_ALLOWED_PRINTING_MODES" == "DISABLED,MOCK" ]] || die "Staging runtime printing modes must be exactly DISABLED,MOCK"
