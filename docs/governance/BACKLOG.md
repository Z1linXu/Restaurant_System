...
| --- | --- | --- | --- | --- | --- | --- | --- |
| UBER-SANDBOX | FEATURE | EXTERNAL_CONFIGURATION_BLOCKED | P1 | Phase B / Uber Eats v1 | Yes for Production | Configure backend-only rotated credential, verified test Store/order-manager provisioning and public HTTPS callback; run real Sandbox + field printing acceptance. Review-gated merge and isolated Staging/Sandbox are authorized; no Production activation. | [integration](../UBER_EATS_INTEGRATION.md), [acceptance](../UBER_EATS_ACCEPTANCE.md) |
