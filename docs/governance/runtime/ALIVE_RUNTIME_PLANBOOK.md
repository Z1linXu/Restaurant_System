...
| Runtime-sensitive current-main candidate | `9a776d3aaa2c357e1edeac46e54168bda1f5431f` | `MACHINE_VERIFIED` Git Ground Truth | PR #91's lifecycle repair is `IN_MAIN`, excludes WebSocket infrastructure only from the dedicated non-web one-shot profile, and is not yet deployed. The current Owner authorization permits its exact Staging-only rebind/preflight/V10-to-V10 deploy and bounded recovery continuation. |
...
| Current package | Exact `2a6c30a...` remains the deployed Staging runtime at V10; it passed formal preflight/readiness and recovered only the reviewed prior blocked pair before its password-free plan reached `VALIDATED` and exposed the WebSocket lifecycle timeout. PR #91 merged the bounded lifecycle repair at current main `9a776d3...`. The Owner has authorized a continuous Staging-only rebind of each bounded repair SHA in this loop, provided V10/no-pending, isolation, credential contract and Production boundaries remain unchanged. The current reviewed blocked pair remains retained until the new exact image passes Batch A. |
...
| Current permitted work | Under the current continuous Owner authorization, bind current exact `main@9a776d3...`, formal-preflight and deploy only isolated Staging V10-to-V10, collect fresh readiness, recover only the exact reviewed blocked pair, and restart STG-005A with fresh approvals. Same-scope bounded Dependency Repairs may merge, rebind, and continue automatically; any migration, security-contract, product, identity, Production or destructive change is a true Owner Gate. |
