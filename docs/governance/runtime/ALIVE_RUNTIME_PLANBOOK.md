...
| Runtime-sensitive deployed-main floor | `2a6c30a5948882ff8bf1d3808e2970fe5b4a6ae3` | `MACHINE_VERIFIED` Git Ground Truth and Staging-only exact deployment | This exact candidate includes PR #89's non-web request-context repair and PR #90's governance closure. Its password-free STG-005A plan reached `VALIDATED`, but a remaining non-web WebSocket broker kept the JVM alive until the bounded timeout. Any later runtime-sensitive repair must be freshly fetched as a new candidate before another runtime approval. |
...
| Loop status | `STG-008_ONE_SHOT_LIFECYCLE_REPAIR_REQUIRES_NEW_EXACT_SHA_STAGING_REBIND_AND_BLOCKED_STATE_RECOVERY_OWNER_RUNTIME_APPROVAL` |
| Current package | The Owner-authorized continuation bound and deployed exact `2a6c30a...` to isolated Staging at V10, passed formal preflight and fresh readiness, and recovered only the reviewed prior blocked pair. PR #89's request-context repair was thereby runtime-validated: the fresh password-free STG-005A plan emitted `VALIDATED` before credential/data access. A non-web WebSocket broker then kept the one-shot alive until its 600-second timeout; Compose began its own `--rm` cleanup while the launcher finalizer attempted its bounded scoped cleanup, so the finalizer fail-closed and retained a new exact blocked pair. Topology/request state remained zero, Staging recovered to 200/200/200 with Printing disabled, and Production continuity remained unchanged. The lifecycle repair is repository-only until a new exact candidate is separately authorized. |
...
| AL-003 state | PRs #61-#87, #89 and #90 are `IN_MAIN`; exact `2a6c30a...` is `DEPLOYED_TO_STAGING` at Flyway V10 after the authorized continuation. STG-008 has no bootstrap execute, credential, source creation, login, onboarding, clone, or acceptance result. The password-free plan now reaches `VALIDATED`, proving PR #89 in the exact runtime, but it cannot exit because the non-web profile still creates the long-lived WebSocket broker. Fail-closed records are retained pending a fresh exact release containing the lifecycle repair and a new Owner runtime gate. |
...
| Current permitted work | Repair the bounded non-web one-shot lifecycle defect through the Dependency Repair Auto-Loop. After that repair enters `main`, obtain a new exact-SHA Owner runtime gate before any release/private-env binding, deploy, readiness, recovery or retry; until then, only Git/GitHub and repository verification are allowed. |
