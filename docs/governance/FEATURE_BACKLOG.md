...
| priority | `HIGH` |
| status | `STG-008_RELEASE_REBIND_REPAIR_IN_MAIN_WAITING_FOR_NEW_EXACT_SHA_STAGING_REBIND_AND_BLOCKED_STATE_RECOVERY_OWNER_RUNTIME_APPROVAL` |
| target_loop | `STG-008_SYNTHETIC_TOPOLOGY_AND_SOURCE`; PR #85 startup-safety repair and PR #87 blocked-state-safe release-rebind repair are `IN_MAIN`, while exact-SHA runtime rebind/recovery remains Owner-gated |
| implementation status | PR-A through PR-F, PRs #58-#87, and independent #66 are `IN_MAIN`. Exact `2837ae88...` remains deployed to isolated Staging at Flyway V10 and `STG-007=PASS`; PRs #83/#84/#86 were documentation only, while runtime-sensitive PRs #85/#87 are not deployed. The Owner approved `STG005_OWNER_20260808_R01` without lowering the password guard. The first password-free `bootstrap-plan` one-shot stopped before its command/data path; cleanup succeeded, topology stayed empty, Production continuity stayed unchanged, and fail-closed state was retained. A later authorized continuation reconfirmed that baseline but stopped before Batch A mutation because ordinary release rotation could not legally pass the retained block. PR #87 repairs only that sequencing deadlock. |
| authority | [AL-003A final menu comparison](agile/AL-003A_FINAL_MENU_COMPARISON.md), [AL-003 technical plan](agile/AL-003_STORE_MENU_CLONE_TECHNICAL_PLAN.md), [STG-008 entry evidence](runtime/STG-008_SYNTHETIC_TOPOLOGY_SOURCE_NO_GO_EVIDENCE.md), [STG-008 Flyway guard repair evidence](runtime/STG-008_STAGING_SYNTHETIC_FLYWAY_GUARD_REPAIR_EVIDENCE.md), and [STG-008 release-rebind repair evidence](runtime/STG-008_RELEASE_REBIND_SERIALIZATION_REPAIR_EVIDENCE.md) |
| next action | Request a new Owner-approved, freshly fetched exact-main candidate that contains PR #87 for recovery-specific release/private-env preparation, formal preflight and Staging-only V10-to-V10 deploy; only after Batch A passes may the separately bounded blocked-state recovery run. Do not reuse the authorization bound to `4759a23b...`, request the runtime-only password before recovery/fresh PLAN, patch the old image, or begin source-menu/login/onboarding/clone/Production work. |
...
| STG-008 release-rebind serialization repair / PR #87 | `IN_MAIN` at `4b954e09a365fec909ed6da3ddf8fa9f13639cdc` | Dedicated recovery release/env preparation preserves both blocked records and every ordinary action block; not deployed or used at runtime |
