...
| priority | `HIGH` |
| status | `STG-008_ONE_SHOT_LIFECYCLE_REPAIR_REQUIRES_NEW_EXACT_SHA_STAGING_REBIND_AND_BLOCKED_STATE_RECOVERY_OWNER_RUNTIME_APPROVAL` |
| target_loop | `STG-008_SYNTHETIC_TOPOLOGY_AND_SOURCE`; exact `2a6c30a...` validated PR #89 in Staging but exposed the remaining one-shot lifecycle defect. |
| implementation status | Exact `2a6c30a...` was authorized and deployed to isolated Staging at Flyway V10, then passed formal preflight and readiness with Printing disabled and unchanged Production continuity. The recovered password-free `bootstrap-plan` emitted `VALIDATED` before credential/data access, but the non-web WebSocket broker held the JVM open through the 600-second bound. The launcher retained a new exact blocked pair after its scoped cleanup raced Compose `--rm`; all synthetic topology/request state remained zero. The bounded lifecycle repair excludes WebSocket infrastructure only from the dedicated non-web one-shot profile; it is not deployed until a new exact candidate receives another Owner runtime gate. |
...
| next action | Complete the lifecycle repair through the repository review/auto-merge policy. Then request a new Owner-approved exact-main release/private-env binding, formal preflight, Staging-only V10-to-V10 deploy, fresh readiness, and bounded recovery of the newly retained records before another PLAN. Do not request the runtime-only password, reuse `2a6c30a...` evidence, begin execute/replay/source-menu/login/onboarding/clone, or perform Production work. |
