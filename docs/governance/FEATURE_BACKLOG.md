...
| priority | `HIGH` |
| status | `STG-008_NON_WEB_REQUEST_CONTEXT_REPAIR_REQUIRES_NEW_EXACT_SHA_STAGING_REBIND_AND_BLOCKED_STATE_RECOVERY_OWNER_RUNTIME_APPROVAL` |
| target_loop | `STG-008_SYNTHETIC_TOPOLOGY_AND_SOURCE`; the repository-only request-context repair must publish before another exact-SHA Staging continuation |
| implementation status | Exact `6753855497...` was authorized, deployed to isolated Staging at Flyway V10, and passed rebind/preflight/readiness/recovery with Printing disabled and unchanged Production continuity. The next password-free `bootstrap-plan` failed before command/data access because a singleton request-context service required a servlet request in non-web mode. Cleanup and zero-write continuity passed and a new blocked pair was retained. The bounded repair changes only optional request injection and is not deployed. |
| authority | [AL-003A final menu comparison](agile/AL-003A_FINAL_MENU_COMPARISON.md), [AL-003 technical plan](agile/AL-003_STORE_MENU_CLONE_TECHNICAL_PLAN.md), [STG-008 entry evidence](runtime/STG-008_SYNTHETIC_TOPOLOGY_SOURCE_NO_GO_EVIDENCE.md), [STG-008 Flyway guard repair evidence](runtime/STG-008_STAGING_SYNTHETIC_FLYWAY_GUARD_REPAIR_EVIDENCE.md), [STG-008 release-rebind repair evidence](runtime/STG-008_RELEASE_REBIND_SERIALIZATION_REPAIR_EVIDENCE.md), and [STG-008 non-web request-context repair evidence](runtime/STG-008_NON_WEB_REQUEST_CONTEXT_REPAIR_EVIDENCE.md) |
| next action | Publish and independently review the bounded repair. Its runtime-sensitive merge creates a new exact candidate; request a new Owner-approved exact-main release/private-env binding, formal preflight, Staging-only V10-to-V10 deploy, fresh readiness, and bounded recovery of the newly retained records before another PLAN. Do not request the runtime-only password, reuse `6753855...` evidence, begin execute/replay/source-menu/login/onboarding/clone, or perform Production work. |
...
| Synthetic Organization/source/Owner bootstrap | `STAGING_DEPENDENCY_REPAIR_RUNTIME_GATE` | Owner identity/password contract is aligned. Exact `6753855497...` completed rebind/deploy/readiness and old-pair recovery. Its next password-free plan failed before the STG-005A command because non-web startup required servlet request injection. Cleanup and zero-write evidence passed; the new blocked state is retained. Before retry, publish the bounded repair, approve its fresh exact main for recovery-specific release/preflight/Staging deploy, and retain Batch A-before-recovery ordering. |
